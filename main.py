...
class DynamicAllocator(AgentBroker):
    def __init__(s): super().__init__(); s.fb={}; s.w={'c':1.0,'w':0.5,'p':0.3}
    ...
    def _sc(s,a,t):
        ag,tsk=s.agents[a],s.tasks[t]
        w=1.0/(1+len(ag['tasks'])); pf=s.fb.get(a,{}).get(t,2.5)/5.0
        return (1.0*s.w['c'] + w*s.w['w'] + pf*s.w['p']) * tsk['priority']
    def rebalance(s):
        for a in s.agents.values(): a['tasks']=[]
        for t in s.tasks.values(): t['a']=None
        for tid in sorted(s.tasks, key=lambda x:s.tasks[x]['priority'], reverse=1): s._a(tid)
...
def main():
    p=argparse.ArgumentParser(description='Dynamic Multi-Agent Task Allocation System', epilog='Ex: %(prog)s -a a1 -c python,api -T t1 -r python,api -S')
    p.add_argument('-a','--agent'); p.add_argument('-c','--cap',default=''); p.add_argument('-T','--task'); p.add_argument('-r','--req',default=''); p.add_argument('-p','--pri',type=int,default=1)
    p.add_argument('-R','--rebalance',action='store_true'); p.add_argument('-S','--status',action='store_true'); p.add_argument('-j','--json',action='store_true')
    p.add_argument('-f','--feedback',nargs=3,metavar=('A','T','R'))
    args=p.parse_args(); b=DynamicAllocator()
    if args.agent:
    ...
    if args.feedback:
        a,t,r=args.feedback
        if not b.feedback(a,t,float(r)): sys.exit(1)
        print(f"Feedback: {a}->{t}={r}")
    if args.status:
        s=b.status()
        print(json.dumps(s, indent=2) if args.json else f"Agents: {s['agents']}\nTasks: {s['tasks']}\nAssignments: {s['assigned']}/{s['tasks']}")
